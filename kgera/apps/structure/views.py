...
def housetype_dashboard(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            ht = HouseType.objects.create(htype=htype, is_block=is_block, block_count=block_count, is_flat=is_flat, flat_count=flat_count)
            ht.save()
...
def commtype_dashboard(request):
    ...
    if request.method == "POST":
        ...
        return HttpResponseRedirect(reverse("commtype_dashboard"))
...
def communities_dashboard(request):
    ...
    if request.method == "POST":
        ...
        return HttpResponseRedirect(reverse("communities_dashboard"))
    ...
    if request.method == "POST":
        ...
        if approval:
            ...
            for community in all_communities:
                #Generator
                if community.communitytype.housetype.is_block == True:
                    if community.communitytype.housetype.is_flat == True:
                        #With Flats
                        ...
                        for block in range(1,block_count+1):
                            ...
                            for flat in range(1, flat_count+1):
                                ...
                                if Houses.objects.filter(housecode=h_code).exists():
                                    ...
                                    house = Houses.objects.create(community=community, housecode=h_code, housestatus=h_status)
                                    house.save()
                                    total_houses += 1
                    else:
                        #Without Flats
                        ...
                        for block in range(1,block_count+1):
                            ...
                            ##########################
                            h_code = f"{community.commcode}/{char1}{char2}{char3}{char4}/{b_code}"
                            h_status = 0
                            if Houses.objects.filter(housecode=h_code).exists():
                                pass
                            else:
